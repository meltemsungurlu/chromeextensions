...
chrome.runtime.onMessage.addListener(( /** @type {pipeRequest} */ message, sender, sendResponse) => {
    console.log('%cResponse', 'color:blue;font-size:20px;padding:50px;', message);

    if(message.request && message.request.requestType==="phone-message"){
        /** @type {messageRequest}*/
        let messageRequest=message.request;
         if(messageRequest.messageType=='phone-message'){
        sendMessageType(messageRequest);
   
        
           sendResponse({
               success:true,
               farewell: "Phone number received ans processed:" + messageRequest.recipient,
           });
       return true;
       }
    }
    ...
    api :{
        whatsApp:{
             ...
             openShortcutsDialog: (contact) => {
                ...
                let build = (e) => {
                    let label = ``;
                    var link = ayanoglu.chrome.api.whatsApp.makeWinShortcut(contact.phone1Value, nameElement.value, txtElement.value, ck.checked);
         ...
         parseMemberInfo : function() {
                ...
                ayanoglu.chrome.api.whatsApp.parseContactInfo().then((contact) => {
                    console.dir(contact);
            
                    if (contact.saved) {
                        ayanoglu.chrome.api.whatsApp.openShortcutsDialog(contact);
                    } else {
         ...
         makeShortcuts : function()  {
                ...
                ayanoglu.chrome.api.whatsApp.parseContactInfo().then(ayanoglu.chrome.api.whatsApp.openShortcutsDialog)
         ...
         collectUnknownNumbers:function(filter) {
                ...
                var stack = [];
                var groupIndex = 1;
                ayanoglu.chrome.api.whatsApp.iterateUsers((contact) => {
                    if (contact === false) {
                    ...
                    if (filter(contact)) {
                        ...
                        var link = ayanoglu.chrome.api.whatsApp.makeWinShortcut(contact.phone);
            ...
            parseContactInfo : function() {
                ...
                return new Promise((resolve, reject) => {
                    ...
                    ayanoglu.chrome.api.whatsApp.openChatPanel().then((element) => {
         ...
         collectWupGroupMembers : function() {
                ...
                let collectMembers = () => {
            let contacts=[];
                    ayanoglu.chrome.api.whatsApp.collectGroupMembers(contacts).then((members) => {
                        contacts = members;
                        window.csvStack = [];
            ...
            ,
             ...
             collectGroupMembers : function(membersArr) {
                ...
                return new Promise((finalResolve, finalReject) => {
                    ...
                    ayanoglu.chrome.api.whatsApp.openChatPanel().then(() => {
                    ...
                    if (nameNode = row.querySelector("div.name")) {
                        ...
                        if (phoneNode = row.querySelector("div:nth-child(5) > a")) {
                            ...
                            if (['Reyaphasta', 'Kolanhasta'].indexOf(compNode.textContent) !== -1)
                                lines.push(ayanoglu.chrome.api.whatsApp.makeWinShortcut(phoneNode.textContent, nameNode.textContent + ' - ' + compNode.textContent));
                        }
                    }
                });
...
let sendMessageType = ( /** @type {messageRequest} */ request) => {
    var link = ayanoglu.chrome.api.whatsApp.getProfileUrl(request.recipient);

    //905332761903
    var url = 'https://' + link ;
    url += request.message ? '&text=' + encodeURI(request.message):'';
...
let onLoad = () => {
    ...
    pop.add('Open Group Member Collector', () => {
        ayanoglu.chrome.api.whatsApp.collectWupGroupMembers()
    }, 'icon-doc-text');


    pop.add('Make Shortcuts', () => {
        ayanoglu.chrome.api.whatsApp.makeShortcuts();
    }, 'icon-export');
    ...
    pop.add('Collect Unknown Numbers', () => {
        ayanoglu.chrome.api.whatsApp.collectUnknownNumbers();
    }, 'icon-attach');
...
const popPanel = ayanoglu.ui.controls.selectionPop((text) => {
    console.log(text);
    ayanoglu.chrome.api.whatsApp.sendMessage(text);
}, true);
