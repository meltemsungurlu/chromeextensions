...
let collectContactsWupLinks = function() {
        ...
        if (nameNode = row.querySelector("div.name")) {
            ...
            if (phoneNode = row.querySelector("div:nth-child(5) > a")) {
                ...
                if (['Reyaphasta', 'Kolanhasta'].indexOf(compNode.textContent) !== -1)
                    lines.push(makeWinShortcut(phoneNode.textContent, nameNode.textContent + ' - ' + compNode.textContent));
            }
        }
    });
...
let sendMessage = function(message) {
    ...
    //CHROME 
const makeWinShortcut = function(phone, name, more, isTel = false) {
    name = name || 'Untitled';
    more = more || '';
    var wPhone = phone.replace(/[^\d]/g, '');
...
let getProfileUrl = function(phone) {
...
let makeShortcut = function(label) {
    // console.clear();
...
const openShortcutsDialog = (contact) => {
    ...
    let build = (e) => {
        let label = ``;
        var link = makeWinShortcut(contact.phone1Value, nameElement.value, txtElement.value, ck.checked);
...
let makeShortcuts = () => {
    ...
    parseContactInfo().then(openShortcutsDialog)
...
function collectUnknownNumbers(filter) {
    ...
    iterateUsers((contact) => {
        ...
        if (filter(contact)) {
            ...
            var link = makeWinShortcut(contact.phone);
...
let collectWupGroupMembers = function() {
    ...
    let collectMembers = () => {
        ...
        collectGroupMembers(contacts).then((members) => {
            contacts = members;
            window.csvStack = [];
            contacts.forEach((contact) => {
...
const sendContactToGoogle=(menu, info, tab) => {
...
const sendSelectionToWhatsApp= (menu, info, tab)=>{ 
...
const searchPhoneInGoogleContacts= (menu, info, tab)=>{ 
