...
chrome.runtime.onMessage.addListener(
  function ( request, sender, sendResponse ) {
    ...
    console.log('%cReceived message' ,'color:white;font-size:16px;padding:20px;background-color:navy',  sender.tab ?
      "from a content script:" + sender.tab.url :
      "from the extension" );
    console.log( request );

if(request.name === 'phone-number'){
  console.log('%cPhone request', 'color:red;font-size:16px;padding:20px;background-color:pink', request);
  return true;
}
